...
CONDITIONS_MAP = {
        ...
        "40+ WAR Season": "SUM((((b.b_H + b.b_BB - IFNULL(b.b_CS, 0) + IFNULL(b.b_HBP, 0)) * ((b.b_H - b.b_2B - b.b_3B - b.b_HR) + (2 * b.b_2B) + (3 * b.b_3B) + (4 * b.b_HR) +  (0.26 * (b.b_BB - IFNULL(b.b_IBB, 0) + IFNULL(b.b_HBP, 0))) + (0.52 * (IFNULL(b.b_SH, 0) + IFNULL(b.b_SF, 0) + IFNULL(b.b_SB, 0)))))/ (b.b_AB + b.b_BB + IFNULL(b.b_HBP, 0) + IFNULL(b.b_SH, 0) + IFNULL(b.b_SF, 0)))) / 10 >= 40",
        ...
        "First Round Draft Pick": "EXISTS (SELECT 1 FROM collegeplaying cp WHERE cp.playerID = b.playerID AND cp.yearID = (SELECT YEAR(MAX(p.debutDate)) FROM people p WHERE p.playerID = b.playerID))",
